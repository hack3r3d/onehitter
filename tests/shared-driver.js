...
describe('currentDriver (shared.js line 6)', () => {
  ...
  afterEach(() => {
    if (prev == null) delete process.env.DB_DRIVER
    else process.env.DB_DRIVER = prev
  })

  it('returns "sqlite" when DB_DRIVER=sqlite', () => {
    process.env.DB_DRIVER = 'sqlite'
    assert.strictEqual(currentDriver(), 'sqlite')
  })

  it('returns "mongodb" by default and when DB_DRIVER is anything else', () => {
    delete process.env.DB_DRIVER
    assert.strictEqual(currentDriver(), 'mongodb')

    process.env.DB_DRIVER = 'not-sqlite'
    assert.strictEqual(currentDriver(), 'mongodb')
  })
})
